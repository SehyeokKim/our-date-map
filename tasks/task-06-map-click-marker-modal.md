...
- 저장된 핀 상세 보기 시 **핀 삭제(Spot Deletion)** 기능 및 제목이 `"Test"`(대소문자 미구분)인 테스트 기록에 대한 **3분 후 자동 삭제 타이머** 기능을 제공합니다.
...
- **`useKakaoMap.ts`**:
  ...
  - 지도 클릭 시 `geocoder.coord2Address()`를 통한 실시간 Reverse Geocoding 실행 및 오버레이 핀 배치.
- **`useDateSpots.ts`**:
  - Supabase DB 및 Storage 이미지 파일 삭제가 포함된 `deleteDateSpot` 기능 구현.
  - `"Test"` 제목 작성 시 3분(180초) 자동 삭제 타이머(`scheduleTestAutoDelete`) 및 패칭 시 만료 항목 자동 정리 로직 구축.
...
- **`src/components/modal/SpotDetailSheet.tsx`**:
  - 데이트 장소 상세 보기 시트 하단에 🗑️ **핀 삭제** 버튼 추가 및 confirm 대화상자 적용.
...
- `[NEW] src/components/modal/SpotDetailSheet.tsx`: 핀 삭제 기능이 포함된 상세 보기 바텀 시트
