...
🎯 **목표 (Goal)**
- 데이트 기록 등록 시 **최대 10장의 사진 다중 선택 및 자동 압축 업로드**를 지원하고, PostgreSQL 데이터베이스 스키마(`image_urls TEXT[]`) 및 Supabase Storage 파이프라인을 다중 이미지에 맞게 구현합니다.
...
- **`SpotSummarySheet.tsx`**: 1단계 요약 화면 (대표 사진 1장, 1줄 메모 미리보기, 클릭 가능한 제목 링크, 핀 삭제 버튼 없음).
- **`SpotDetailSheet.tsx`**: 2단계 전체 자세히보기 화면 (최대 10장 갤러리 슬라이더 캐러셀, 메모 전문, 🗑️ 핀 삭제 버튼).
