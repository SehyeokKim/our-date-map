...
- 저장된 핀 터치 시 **2단계 상세 보기 프로세스 (1단계: 요약 정보 팝업 → 2단계: 전체 상세 팝업)**를 제공하며, 전체 상세 보기에서 **핀 삭제(Spot Deletion)** 기능 및 제목이 `"Test"`(대소문자 미구분)인 테스트 기록에 대한 **3분 후 자동 삭제 타이머** 기능을 제공합니다.
...
- **`src/components/modal/SpotSummarySheet.tsx`**:
  - 1단계 요약 정보 팝업 시트 (장소명, 날짜, 데이트 메모 요약 문구, "자세히 보기 →" 클릭 시 2단계 이동).
- **`src/components/modal/SpotDetailSheet.tsx`**:
  - 2단계 전체 상세 보기 시트 (전체 사진, 메모 전문, 주소, 📍 위경도, 🗑️ 핀 삭제 버튼).
...
- `[NEW] src/components/modal/SpotSummarySheet.tsx`: 1단계 요약 정보 팝업 시트
- `[NEW] src/components/modal/SpotDetailSheet.tsx`: 2단계 전체 상세 보기 시트 (핀 삭제 포함)
...
- [x] **2단계 상세 보기 UX:** 핀 클릭 시 1단계 요약 시트 렌더링 → 제목/자세히 보기 터치 시 2단계 전체 시트 전환.
