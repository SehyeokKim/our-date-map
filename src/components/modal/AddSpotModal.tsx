...
export const AddSpotModal: React.FC<AddSpotModalProps> = ({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    // Fallback: If title is empty, automatically use the extracted road address or default text
    const finalTitle = title.trim() || initialAddress || "추억의 데이트 장소";
        ...
        <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              id="title"
              type="text"
              placeholder={initialAddress || "어디서 데이트를 하셨나요?"}
              value={title}
